...
describe("ProviderCommandReactor", () => {
  ...
  it("records conditional delivery without starting a different session and replays without resending", async () => {
    ...
    expect(harness.startSession).not.toHaveBeenCalled();
    expect(harness.sendTurn.mock.calls[0]?.[0]).toMatchObject({ expectedSession });
    await harness.runEffect(harness.engine.dispatch(command));
    await harness.drain();
